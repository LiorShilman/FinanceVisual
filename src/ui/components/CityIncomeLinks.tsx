...
// the corner crisp instead of a CatmullRom curve rounding it off.
function buildElbowGeometry(fromX: number, fromZ: number, toX: number, toZ: number): THREE.TubeGeometry {
  const start = new THREE.Vector3(fromX, Y, fromZ);
  const end = new THREE.Vector3(toX, Y, toZ);
  ...
  if (Math.abs(fromX - toX) < 0.01 || Math.abs(fromZ - toZ) < 0.01) {
    ...
    const corner = new THREE.Vector3(toX, Y, fromZ);
