...
export function CityCrystalMesh({ x, z, height, footprint, color, name, amount, onOpen }: Props) {
  ...
  // the crown mouth (registers its refs first), branch mouths follow in branch order.
  const renderMouth = (radius: number, refIndex: number) => {
    ...
    <group position={[x, 0, z]}>
      ...
      {branches.map((b, i) => (
        <group key={i} position={[0, trunkHeight * 0.75, 0]} rotation={[b.tilt, b.yaw, 0]}>
          ...
          <group position={[0, b.len, 0]}>{renderMouth(b.mouthRadius, i + 1)}</group>
        </group>
      ))}

      <group position={[0, trunkHeight, 0]}>{renderMouth(crownMouthRadius, 0)}</group>
